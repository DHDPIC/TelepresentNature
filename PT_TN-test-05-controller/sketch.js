...
let sendTopic = '_david/PotentiometerValue/';
...
function setup() {
  ...
  for (let r = 0; r < table.getRowCount(); r++) {
    ...
    if(lng == 0) {
      dataXArray[r] = dataXArray[r-1];//10.9;
      dataYArray[r] = dataYArray[r-1];//45.9;
    } else {
...
function draw() {
  ...
  let myAngle = degrees(cubeQ?.angle);
  if(myAngle < 45 && myAngle > -45 || myAngle > 135 || myAngle < -135) {
    print("horizontal");
    timePlay();
  } else {
    print("vertical");
  ...
  let messageToSend = finalTemp.toFixed(1) + "|" + finalAlti.toFixed(1) + "|" + round(red(cubeColor)) + "|" + round(green(cubeColor)) + "|" + round(blue(cubeColor));
...
function timePlay() {
  ...
  cubeQ?.moveTo( {x:xCon, y:yCon}, spd );
  print(cubeQ?.x);
